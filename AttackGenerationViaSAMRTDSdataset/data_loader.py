...
class data_loader_FDIPhy(object):
    ...
    def state_estimation(self, Y_norm_sparse, sensor_location_indices):
        ...
        for t in range(num_time):
            noise_now = 0.001 / np.sqrt(2) * (
                np.random.normal(size=(2*num_sensor, 1)) + 1j * np.random.normal(size=(2*num_sensor, 1))
            )
            for s in range(num_sample):
                ...
                measurement = system_matrix @ voltage_ordered + noise_now
