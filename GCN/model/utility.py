...
def makegso(edge_index, edge_weight, nodes_num):
    edge_index_np = edge_index.cpu().detach().numpy()
    edge_weight_np = edge_weight.cpu().detach().numpy().reshape(-1,)
    row = edge_index_np[0,:]
    col = edge_index_np[1,:]
    # print('xxx', edge_weight_np.shape)
    # print('xxxxx', row.shape)
    # print('xxxxxxx', nodes_num)
    # print('xxxxxxxxx', np.max(col))
    csc_mat_GSO = csc_matrix((edge_weight_np, (row, col)), shape=(nodes_num, nodes_num))
    return csc_mat_GSO

def calc_gso(edge_index, edge_weight, nodes_num, gso_type):
    gso = makegso(edge_index, edge_weight, nodes_num)
